# System Architecture - Cognifyr AI Workflow Ops
...
flowchart TD
    ...
    Worker -->|5. Trigger analysis| AI[Claude AI / Mock Classifier]
...
1. **Immediate response time:** The customer or inbound Webhook (like WhatsApp) never blocks on heavy third-party AI execution. The API responds with `210 Created` within milliseconds, ensuring we don't encounter connection timeouts under load.
2. **Robust background worker queue:** BullMQ manages background processes using Upstash Redis. If the Claude API key is exhausted or has service disruptions, jobs fail and trigger automatic exponential backoff retries without blocking active dashboard sessions.
